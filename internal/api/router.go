...
func NewRouter(svc *library.Service, curationSvc *curation.Service, settingsRepo *repositories.SettingsRepository, lastfmSyncHandler *LastfmSyncHandler, localMode bool) *gin.Engine {
	gin.SetMode(gin.ReleaseMode)
	r := gin.New()
	r.Use(gin.Recovery())
	r.Use(loggingMiddleware())

	sh := NewSettingsHandler(settingsRepo, localMode)

	api := r.Group("/api")
	{
		api.GET("/settings", sh.Get)
		api.PATCH("/settings", sh.Update)
